...
const aiRecommendations_1 = require("../lib/aiRecommendations");
const aiGardeningService_1 = require("../services/aiGardeningService");
const prisma = new client_1.PrismaClient();
const router = express_1.default.Router();
// Middleware to track request start time
const trackStartTime = (req, res, next) => {
...
router.post('/recommendations', trackStartTime, async (req, res) => {
    try {
        ...
        const result = await (0, aiRecommendations_1.getSmartPlantRecommendations)(userPreferences);
        res.json({
...
router.post('/balcony-design', trackStartTime, async (req, res) => {
    try {
        ...
        const result = await (0, aiRecommendations_1.optimizeBalconyDesign)(balconyConfig);
        res.json({
            success: result.success,
            data: result.data,
            message: result.message,
...
router.post('/diagnose', trackStartTime, async (req, res) => {
    try {
        ...
        const result = await (0, aiRecommendations_1.diagnosePlantHealth)(plantId, symptoms, environment);
        res.json({
            success: result.success,
            data: result.data,
            message: result.message,
...
// 辅助函数：生成相容性建议
function generateCompatibilityRecommendations(plants, conditions) {
...
router.post('/diagnose-problem', trackStartTime, async (req, res) => {
    try {
        ...
        const result = await aiGardeningService_1.aiGardeningService.diagnosePlantProblem(plantId, problemDescription, imageBuffer);
        res.json({
            success: true,
            data: result,
            message: '完成植物问题诊断',
...
router.post('/personalized-care', trackStartTime, async (req, res) => {
    try {
        ...
        const result = await aiGardeningService_1.aiGardeningService.getPersonalizedCareRecommendations(userId, plantId);
        res.json({
            success: true,
            data: result,
            message: plantId
