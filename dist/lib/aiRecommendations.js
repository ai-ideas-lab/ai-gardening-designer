...
exports.getSmartPlantRecommendations = getSmartPlantRecommendations;
exports.optimizeBalconyDesign = optimizeBalconyDesign;
exports.diagnosePlantHealth = diagnosePlantHealth;
 ...
 */
async function getSmartPlantRecommendations(userPreferences) {
    try {
        // 获取所有植物数据
        ...
        if (userPreferences.balconyDirection === 'north') {
            filteredPlants = filteredPlants.filter(plant => plant.light === 'low' || plant.light === 'partial-shade');
        }
        else if (userPreferences.balconyDirection === 'south') {
            filteredPlants = filteredPlants.filter(plant => plant.light === 'full-sun' || plant.light === 'partial-sun');
        }
        // 计算每个植物的推荐分数
        const scoredPlants = filteredPlants.map(plant => {
            ...
            if (userPreferences.preferences) {
                ...
                const hasPreferredBenefit = userPreferences.preferences.some(pref => benefits.includes(pref));
 ...
 */
async function optimizeBalconyDesign(balconyConfig) {
    try {
        // 获取适合的植物列表
        ...
        // 预算分配
        const totalPlantCount = design.layout.primaryZones.reduce((sum, zone) => sum + zone.plants.reduce((zoneSum, plant) => zoneSum + plant.quantity, 0), 0);
 ...
 * 植物健康诊断和问题解决方案
 */
async function diagnosePlantHealth(plantId, symptoms, environment) {
    try {
        ...
        // 查询特定植物的养护知识
        const relevantKnowledge = plant.knowledge.filter(knowledge => knowledge.category === 'care' || knowledge.category === 'disease');
        if (relevantKnowledge.length > 0) {
            diagnosis.preventionTips = relevantKnowledge.map(knowledge => knowledge.content).slice(0, 3);
        }
        // 确定紧急程度
